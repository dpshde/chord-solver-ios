...
//  Functional Harmony
//
//  Maps natural-language music queries onto Chords / Scales / Notes state.
//
...
struct MusicQueryMatch: Equatable {
    ...
    let summary: String
    /// Root in app notation (`C`, `C#`, `Bb`). Nil for multi-note Notes only.
    let root: String?
...
enum MusicQueryParser {
    ...
    /// Parse free text into a chord, scale, or Notes note list.
    static func parse(_ raw: String) -> MusicQueryMatch? {
        ...
        if !wantsScale, let noteMatch = parseNoteList(normalized) {
            return noteMatch
        }
    ...
    // MARK: Notes (multi-note identification)

    private static func parseNoteList(_ text: String) -> MusicQueryMatch? {
        // Strip intent words
        var t = text
        ...
        for tok in tokens {
            ...
            if notes.count >= NotesSessionState.maxNotes { break }
        }
...
struct MusicQueryPreview: Equatable {
    /// Primary answer line (chord/scale name, or Notes identify result).
    let title: String
    /// Pitch classes to show as chips.
    let notes: [String]
    /// Optional secondary line (interval formula, Notes detail).
    let detail: String?
}
...
enum MusicQueryPreviewResolver {
    ...
    private static func notesPreview(_ match: MusicQueryMatch) -> MusicQueryPreview {
        let notes = match.noteList ?? []
        let answer = NotesChordIdentifier.identify(notes: notes)
