...
private func getDistance(_ a: String, _ b: String) -> Int {
    guard let first = a.first, let second = b.first,
          let firstNote = NotesResources.baseNotes[String(first)],
          let secondNote = NotesResources.baseNotes[String(second)] else {
        return 0
    }
...
private func getIntervalSemitones(_ a: String, _ b: String) -> Int? {
    guard let va = NotesResources.dictionaryNotes[a],
          let vb = NotesResources.dictionaryNotes[b] else {
        return nil
    }
    let inter = va - vb
...
private func identifyInterval(_ a: String, _ b: String) -> String? {
    ...
    guard let candidates = NotesResources.intervals[distance] else { return nil }
    return candidates.first(where: { $0.value == interval })?.name
}

// MARK: - Chord identification

private enum NotesIdentifyError: Error {
    case noChord
}

private func identifyChord(_ args: [String]) throws -> String {
    var notes = args
    guard notes.count == 3 || notes.count == 4 else { throw NotesIdentifyError.noChord }

    // Sort upper notes by letter distance from the first note (letter-distance order).
    let aLetter = String(notes[0].prefix(1))
    guard let aIndex = NotesResources.noteArray.firstIndex(of: aLetter) else {
        throw NotesIdentifyError.noChord
    }

    func adjustedIndex(_ note: String) -> Int {
        let letter = String(note.prefix(1))
        guard var idx = NotesResources.noteArray.firstIndex(of: letter) else { return 0 }
        if idx < aIndex { idx += 9 }
        return idx
    }
    ...
    case 3:
        ...
        guard let firstInterval = identifyInterval(a, b),
              ...
              let chord = NotesResources.threeNoteChords[firstInterval]?[secondInterval] else {
            throw NotesIdentifyError.noChord
        }
    ...
    case 4:
        ...
        guard let firstInterval = identifyInterval(a, b),
              ...
              let chord = NotesResources.fourNoteChords[firstInterval]?[secondInterval]?[thirdInterval] else {
            throw NotesIdentifyError.noChord
        }
...
private func formatThreeNote(a: String, b: String, c: String, chord: NotesChordMatch) -> String {
...
private func formatFourNote(a: String, b: String, c: String, d: String, chord: NotesChordMatch) -> String {
...
// MARK: - Resources (from vivace-theory Resources.ts (upstream))

struct NotesChordMatch {
    let name: String
...
enum NotesResources {
    ...
    static let threeNoteChords: [String: [String: NotesChordMatch]] = [
        "Minor 2nd": [
    ...
    static let fourNoteChords: [String: [String: [String: NotesChordMatch]]] = [
        "Diminished 5th": [
            "Major 2nd": [
